...
import { mockVendors } from '@/data/mock'; // Assuming mockVendors contains all vendor data
...
export default function DashboardPage() {
  ...
  const handleRecommendations = (newRecommendations: VendorRecommendation[]) => {
    ...
    const detailedRecommendations = newRecommendations.map(rec => {
      const vendorDetails = mockVendors.find(v => v.name === rec.vendorName);
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-8">
            {recommendations.map((rec) => (
              rec.details ? <VendorCard key={rec.details.id} vendor={rec.details} matchScore={rec.matchScore * 100} /> : null
            ))}
          </div>
        </section>
      )}
